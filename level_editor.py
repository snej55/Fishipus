...
class Editor(Pengine):
    ...
    def handle_lry_panel(self, mouse_pos):
        ...
        if panel_rect.collidepoint(mouse_pos[0], mouse_pos[1]):
            if self.scrolling:
                self.lry_panel_scroll = max(0, self.lry_panel_scroll + self.scrolling * 10)
    ...
    def update(self):
        ...
        if pygame.K_n in self.toggles and self.keys[pygame.K_LCTRL]:
            self.tile_map.layers.append(Layer({'tile_map': {}, 'decor': [], 'solid': False, 'render_scale': 1.0}, self, mode='edit', index=len(self.tile_map.layers)))
