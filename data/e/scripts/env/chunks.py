...
class Rects:
    ...
    def load_chunks(items, chunk_dim):
        chunk_data = {}
        for item in items:
            loc = str(math.floor(item[0].x / chunk_dim[0] / TILE_SIZE)) + ';' + str(math.floor(item[0].y / chunk_dim[1] / TILE_SIZE))  # item must have attribute pos as a pygame.Vector2 or list
    ...
    def add_item(self, item):
        loc = str(math.floor(item[0].x / self.chunk_dim[0] / TILE_SIZE)) + ';' + str(math.floor(item[0].y / self.chunk_dim[1] / TILE_SIZE))  # item must have attribute pos as a pygame.Vector2 or list
